...
def generate_continuous_trajectory(points, q_start, v_max=MAX_LINEAR_SPEED):
    ...
    while dist < total_dist:
        ...
        L_vec = J_inv @ u_hat #Directional Joint Mapping Ratio
        ...
        for i in range(3):
            l = L_vec[i] #in rad/cm
            bound_upper = dq_curr[i] + MAX_JOINT_ACC * DT #in rad/s
        ...
        v_actual = min(v_desired, v_vlim, v_max_allowed) #all are in cm/s one is desired,one is for max-velo and one for max-accln
        v_actual = max(v_actual, v_min_allowed)
        
        # Maintain a reasonable minimum speed (0.5 cm/s) so it doesn't freeze at the end of lines
        v_actual = max(v_actual, 0.5) #insures arm doesnt come to zero velocity
...
class Planar3DOFSimApp:
    ...
    def update_preview(self, event=None):
        ...
        self.status_var.set("STATUS: CHECK OPENCV DEBUG WINDOW...")
        self.root.update()

        # --- HELPER FUNCTION: Pause and Wait (GUI Safe) ---
        def show_and_wait(step_name, image):
            """Displays the image and pauses until Space or Enter is pressed.
               Keeps Tkinter GUI alive during the wait loop."""
            print(f"Showing: {step_name} | Press SPACE or ENTER in the image window to proceed...")
            cv2.imshow("CV Pipeline Debugger", image)
            
            while True:
                # waitKey(10) waits 10ms for CV2 events
                key = cv2.waitKey(10) & 0xFF 
                if key == 32 or key == 13: # 32=Space, 13=Enter
                    break
                # Keep Tkinter UI responsive while waiting
                self.root.update()

        # ==========================================
        # 1. Raw Image
        # ==========================================
        show_and_wait("1. Raw Grayscale Image", self.raw_image)

        # ==========================================
        # 2. PORTRAIT PRE-PROCESSING (Bilateral Filter)
        # ==========================================
        face_smoothed = cv2.bilateralFilter(self.raw_image, d=9, sigmaColor=75, sigmaSpace=75)
        show_and_wait("2. Bilateral Filter", face_smoothed)
        
        # ==========================================
        # 3. EDGE DETECTION (Using Sliders)
        # ==========================================
        t1 = self.t1_var.get()
        t2 = self.t2_var.get()
        edges = cv2.Canny(face_smoothed, threshold1=t1, threshold2=t2)
        show_and_wait("3. Canny Edges", edges)
        ...
        show_and_wait("4. Morphological Close", edges)
        ...
        contour_canvas = np.zeros_like(self.raw_image)
        cv2.drawContours(contour_canvas, contours, -1, (255), 1)
        show_and_wait(f"5. Extracted Contours ({len(contours)} found)", contour_canvas)

        # Clean up the debug window before proceeding
        cv2.destroyAllWindows()
        self.status_var.set("STATUS: MAPPING PATHS TO WORKSPACE...")
        self.root.update()
