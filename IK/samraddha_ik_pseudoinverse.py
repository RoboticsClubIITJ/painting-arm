...
class PlanarRobotArm:
    ...
    def _pose_and_jacobian(self, q):
        """
        Given joint angles q (in RADIANS, as a length-3 array), return:
          - pose: the current end-effector [x, y, phi]
          - J:    the 3x3 Jacobian matrix at that pose

        The Jacobian tells us: "if I nudge each joint angle by a tiny
        amount, how much does the end-effector's x, y, and phi change?"
        Each column of J corresponds to one joint; each row corresponds
        to one output (x, y, phi).
        """
        L1, L2, L3 = self.l1, self.l2, self.l3
        t1, t2, t3 = q
        t12 = t1 + t2          # cumulative angle after joint 2
        t123 = t1 + t2 + t3    # cumulative angle after joint 3 (= phi)

        # Forward kinematics in closed form (same math as compute_fk,
        # just written directly in terms of t1, t2, t3 instead of a loop)
        x = L1 * np.cos(t1) + L2 * np.cos(t12) + L3 * np.cos(t123)
        y = L1 * np.sin(t1) + L2 * np.sin(t12) + L3 * np.sin(t123)
        phi = t123
        pose = np.array([x, y, phi])

        # Partial derivatives of x, y, phi with respect to t1, t2, t3.
        # Row 1: dx/dt1, dx/dt2, dx/dt3
        # Row 2: dy/dt1, dy/dt2, dy/dt3
        # Row 3: dphi/dt1, dphi/dt2, dphi/dt3 (always 1,1,1 since
        #        phi = t1+t2+t3 -- each joint contributes equally to
        #        final orientation)
        J = np.array([
    ...
    def compute_ik_pseudoinverse(self, target_x, target_y, target_phi_deg,
        ...
        Iteratively updates joint angles q using:
            q_new = q_old - alpha * pinv(J) @ error

        where `error` is how far off the current end-effector pose is
        from the target pose. Unlike the plain Jacobian-transpose
        gradient descent, the pseudoinverse accounts for how the
        joints interact with each other, so it converges in far fewer
        steps and tolerates a much larger step size (alpha).
        ...
        for i in range(max_iterations):
            ...
            error = pose - target  # how far current pose is from target

            if np.linalg.norm(error) < tolerance:
                return np.degrees(q), True, i

            # pinv(J) approximately "undoes" J, converting a desired
            # change in (x, y, phi) into the change in (t1, t2, t3)
...
if __name__ == "__main__":
    print("--- Bob Ross Robot Arm: Pseudoinverse IK ---")
    ...
    angles, converged, iters = arm.compute_ik_pseudoinverse(
    ...
    if converged:
        ...
        print("Try a smaller alpha or a different initial_guess.")
    ...
    reach = l1 + l2 + l3 + 5
    ax.set(title='Pseudoinverse (Jacobian) IK', xlabel='X (cm)', ylabel='Y (cm)',
