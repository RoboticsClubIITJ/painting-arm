...
def ik_gradient_descent(xd, yd, phid, l1, l2, l3, q0, alpha=0.1, tol=1e-3, max_iter=1000):
    """Jacobian Transpose IK solver using gradient descent."""
    q = np.array(q0, dtype=float)
    target = np.array([xd, yd, phid])

    for i in range(max_iter):
        pose = forward_kinematics(q, l1, l2, l3)
        e = pose - target  # error vector
        if np.linalg.norm(e) < tol:
            break
        J = jacobian(q, l1, l2, l3)
        dq = -alpha * J.T @ e   # gradient descent update
        q += dq
...
if __name__ == "__main__":
    ...
    q_sol, pose_sol = ik_gradient_descent(xd, yd, phid, l1, l2, l3, q0)
