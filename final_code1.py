...
class Planar3DOFSimApp:
    ...
    def setup_gui(self):
        ...
        ttk.Button(control_frame, text="Upload Image & Run", command=self.process_image).pack(pady=20, fill=tk.X)
    ...
    def process_image(self):
        ...
        face_smoothed = cv2.bilateralFilter(img, d=9, sigmaColor=75, sigmaSpace=75)
        
        # 2. EDGE DETECTION (Softer limits)
        edges = cv2.Canny(face_smoothed, threshold1=30, threshold2=90)

        # 3. MORPHOLOGICAL CLOSE (Bridge gaps)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        edges = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, kernel)

        # 4. CONTOUR DETECTION (RETR_LIST to catch interior facial features)
        ...
        for cnt in contours:
            ...
            # 5. SPLINE INTERPOLATION (Smoothes out stair-step pixelation)
            x = cnt[:, 0]
            y = cnt[:, 1]
            ...
            try:
                ...
                # Calculate point density based on the line's actual length
            ...
            pts = smooth_approx.astype(np.int32).reshape((-1, 1, 2))
            cv2.polylines(preview_img, [pts], isClosed=True, color=(0, 255, 0), thickness=2)

        cv2.imshow("Extracted Waypoints (Press ENTER to run)", preview_img)
        cv2.waitKey(0)
    ...
    def generate_full_trajectory(self, cv_paths):
        ...
        for i, ax in enumerate(self.vel_axes):
            max_v = max(MAX_JOINT_VEL, np.max(np.abs(self.traj_dq[:, i])))
