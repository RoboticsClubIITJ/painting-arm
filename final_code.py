...
class Planar3DOFSimApp:
    ...
    def update_preview(self, event=None):
        ...
        # ==========================================
        # === NEW: HISTORY / DUPLICATE CHECK ===
        # ==========================================
        visited_grid = set()
        GRID_SIZE = 3.0  # Pixel tolerance distance for marking an area as 'visited'
        OVERLAP_THRESHOLD = 0.25  # If 50% of the path is already known, skip it
        ...
        for cnt in contours:
            ...
            overlap_count = 0
            for pt in smooth_approx:
                grid_x = int(pt[0] / GRID_SIZE)
                grid_y = int(pt[1] / GRID_SIZE)
                if (grid_x, grid_y) in visited_grid:
                    overlap_count += 1
            
            if len(smooth_approx) > 0:
                overlap_ratio = overlap_count / len(smooth_approx)
                if overlap_ratio > OVERLAP_THRESHOLD:
                    continue # This path covers previously visited coordinates, skip it!
            
            # --- REGISTER PATH TO HISTORY ---
            for pt in smooth_approx:
                grid_x = int(pt[0] / GRID_SIZE)
                grid_y = int(pt[1] / GRID_SIZE)
                visited_grid.add((grid_x, grid_y))
